...
struct ProtocolFrame_PacketParm {
    PProtocolFramePARM pFrame;              /* 框架指针 */
    ...
    /* exitCallPack 退出回调函数，对于Loop来说是退出当前链路功能的Loop，对于CallBack来说，是退出后续所有功能CakllBack，true退出，false不退出 */
    uint8_t exitCallPack: 1;
    /* 结束Loop功能，为true时，多个链路不重复调用同一个Loop,为fasle，每个链路都会调用一次,默认为true */
    uint8_t exitLoop;
};
...
struct SProtocolFrameAPI {
    ...
    ** Input parameters:    pFrame：框架参数结构体指针
    **                      pLink:查找的链路参数结构体指针
    **                      pIndex:接收位置索引的指针,可以为NULL
    ** Output parameters:   no
    ** min：                no
    ** Returned             true，链路存在，false，链路不存在
    *************************************************************/
    uint8_t (*GetLinkInFrame)(PProtocolFramePARM pFrame, PProtocolFrame_LinkPARM pLink, uint8_t *pIndex);
    /*************************************************************
    ...
    **                      false，失败
    *************************************************************/
    bool (*ConnectFLFF)(PProtocolFramePARM pFrame,
    ...
    *************************************************************/
    bool (*DisConnectFLFF)(PProtocolFramePARM pFrame,
                           PProtocolFrame_LinkPARM pLink,
                           PProtocolFrame_FormatPARM pFormat,
                           PProtocolFrame_FuncLIST pFuncList);
    /*************************************************************
