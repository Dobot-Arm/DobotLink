...
/*************************************************************
** Function name:       K210Arm_RegisterCB
** Descriptions:        注册K210Arm功能回调函数
...
** Remarks:             None
*************************************************************/
bool K210Arm_RegisterCB(uint16_t id, DobotV3CmdInter_CB CB)
{
    return gDobotV3CmdInterAPI.RegisterCB(&K210ArmRulesCB, id, CB);
}

/*************************************************************
** Function name:       K210Arm_RulesSort
** Descriptions:        匹配分类
...
*************************************************************/
uint8_t K210Arm_RulesSort(PDobotV3Format_Packet pSendPacket, uint16_t length)
{
    if(pSendPacket->version != DobotV3Format_VER_11) {
        return false;
    }
    if(pSendPacket->cmdSet == DobotV3Format_K210 && pSendPacket->des == DobotV3Dev_K210Arm){
        return true;
    }
    return false;
}
/*************************************************************
** Function name:       K210Arm_RulesMatch
** Descriptions:        指令匹配
** Input parameters:    pSendPacket:发送的数据包
...
*************************************************************/
uint8_t K210Arm_RulesMatch(PDobotV3Format_Packet pSendPacket, PDobotV3Format_Packet pReadPacket)
{
...
/* 定义通用规则回调函数列表参数 */
DobotV3CmdInter_CMDCB_LS_EXPORT(K210ArmRulesCB, GetId, 255)
