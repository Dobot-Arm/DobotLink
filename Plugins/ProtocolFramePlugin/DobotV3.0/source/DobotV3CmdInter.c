...
** Remarks:             None
*************************************************************/
static uint8_t GetCmdSpace(PDobotV3CmdInter_HANDLE pHandle, PProtocolFrame_FuncSTRUCT pFuncStruct)
{
                           ...
                           uint16_t length)
{
    ...
    pPkParm->repeatNum = pParm->defaultRepeatNum;
    pPkParm->timeOut = pParm->defaultTimeOut;
    pPkParm->timeStart = gProtocolFrameAPI.GetStartTime() - pPkParm->timeOut;       /* 保证第一次能发送 */
                        ...
                        uint16_t payloadLen)
{
    ...
    /* 指令规则分类 */
    RuleSort(pHandle->pParm, pHandle->index, pPacket, len);
...
** Returned value:      None
*************************************************************/
static void SetSendParm(PProtocolFrame_FuncSTRUCT pFuncStruct,
                        uint8_t repeatNum,
                        uint32_t timeOut)
{
    ((PDobotV3CmdInterPARM)(pFuncStruct->pFuncParmPtr))->defaultRepeatNum = repeatNum;
    ((PDobotV3CmdInterPARM)(pFuncStruct->pFuncParmPtr))->defaultTimeOut = timeOut;
}
...
void DobotV3CmdInter_Loop(PProtocolFrame_FuncSTRUCT pFuncStruct, PProtocolFrame_PacketPARM pPacketParm)
{
    ...
    /* 判断发送超时 */
    if(gProtocolFrameAPI.IsTimeOut(pPkParm->timeOut, pPkParm->timeStart) == false) {
        return;
    }
    /* 判断重发次数 */
    if(pPkParm->repeatNum == 0) {
        /* 判断指令回调是否执行成功，如果执行了指令回调，则自动释放指令空间 */
        ...
        if(CmdCallBackHandle(pParm, pPkParm, ErrorCB) == false) {
            pPkParm->sendMark = false;
